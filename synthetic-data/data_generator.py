...
try:
  ...
  print(" google-generativeai not installed. Using Faker fallback.")
...
if os.path.exists(SEED_FILE):
  try:
    ...
    if _seed_companies:
      ...
      print(f" No seed companies found in {SEED_FILE}")
  except Exception as e:
    print(f" Failed to load seed companies: {e}")
else:
...
Make it realistic and varied. Return ONLY valid JSON as a single object, no markdown formatting."""
    ...
    print(f" Gemini API error: {e}. Using Faker fallback for this record.")
...
def generate_client_from_seed(index):
  ...
  if not company_name or not industry:
    print(f" Incomplete seed company data at index {index}, skipping")
    return None
...
Return only the text, no formatting."""
      ...
      if "429" in error_msg or "Resource exhausted" in error_msg:
        if is_seed_company and retry_count < max_retries:
          ...
          print(f"    Gemini error (using Faker fallback): {e}")
